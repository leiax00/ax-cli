...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Some(cli::Commands::Env { action }) => match action {
            cli::EnvAction::Add {
                name,
                value,
                desc,
            ...
            cli::EnvAction::Edit {
                ...
                let tag_vec: Option<Vec<String>> = tags
                    .as_ref()
                    .map(|t| t.split(',').map(|s| s.trim().to_string()).collect());
                commands::env::edit(
                    &name,
            ...
            cli::EnvAction::Show { name, tag, all } => {
                commands::env::show(name.as_deref(), tag.as_deref(), all, &config)?
            }
            cli::EnvAction::Pause { names, tag, all } => {
                commands::env::pause(&names, tag.as_deref(), all, &config)?
            }
            cli::EnvAction::Resume { names, tag, all } => {
                commands::env::resume(&names, tag.as_deref(), all, &config)?
            }
        ...
        Some(cli::Commands::Install { extras }) => {
            commands::install::execute(&config, extras)?;
        }
