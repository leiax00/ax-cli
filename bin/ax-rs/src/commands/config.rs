...
use crate::config::{Config, config_dir, expand_home, TEMPLATE_CONFIG_YAML, TEMPLATE_ZSHRC, TEMPLATE_WEZTERM};
...
pub fn push(_config: &Config) -> Result<()> {
    ...
    if output.is_ok() && output.as_ref().unwrap().status.success() {
        ...
        if stderr.contains("no upstream") || stderr.contains("fatal") {
            ...
            println!("   或首次推送：git -C {} push -u origin main", cdir.display());
        } else {
...
fn tar_export(cdir: &std::path::Path, output_path: &std::path::Path, with_binary: bool) -> Result<()> {
...
pub fn import(file: &str, _config: &Config) -> Result<()> {
    ...
    // 解压
    let _ = std::process::Command::new("tar")
        .args(["-xzf", path.to_str().unwrap(), "-C", tmp_dir.to_str().unwrap()])
        .output();
    ...
    let imported_bin = tmp_dir.join("bin").join(if cfg!(windows) { "ax.exe" } else { "ax" });
    if imported_bin.exists() {
        ...
        #[cfg(unix)]
        { let _ = std::process::Command::new("chmod").args(["+x", dest.to_str().unwrap()]).output(); }
